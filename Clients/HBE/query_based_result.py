...
def fetchingData(conn: str) -> pd.DataFrame:
...
def three_column_query(sales_data: pd.DataFrame):
    if not sales_data.empty:
        try:
            ...
            with st.container():
                selection_column = st.columns([2, 2, 2, 2])
                ...
                if selected_column_x:
                    ...
                    if selected_column_y:
                        selected_column_list_3 = selected_column_list.copy()
                        # selected_column_list_3.remove(selected_column_y)
                        selected_color = selection_column[2].selectbox('Select the column', selected_column_list_3)

                if selected_column_x and selected_column_y and selected_color:

                    selected_data = sales_data[[selected_column_x, selected_column_y]]

                    chart_option = ['', 'Bar Chart', 'Line Chart', 'Scatter Chart']
                    select_chart = selection_column[3].selectbox("Select the Chart", chart_option)
            ...
            if selected_column_x and selected_column_y:
                ...
                with chart_tab:
                    ...
                    if select_chart == 'Bar Chart':
                        if selected_color:
                            barChart(selected_data, selected_column_x, selected_column_y, colored=selected_color)
                        else:
                        ...
                        if selected_color:
                            scatterChart(selected_data, selected_column_x, selected_column_y, colored=selected_color)
                        else:
